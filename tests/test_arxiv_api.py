...
class ArxivApiAdapterTest(unittest.TestCase):
    ...
    def test_get_arxiv_data_supports_home_callback_defaults(self, mock_client):
        mock_client.return_value.fetch_articles_xml.return_value = "<feed />"

        xml = get_arxiv_data("machine learning", max_results=5)
        ...
        self.assertEqual(request.search_field, "all")
        self.assertIsNone(request.id_list)
        self.assertEqual(request.start, 0)
        self.assertEqual(request.max_results, 5)
        self.assertEqual(request.sort_by, "submittedDate")
        self.assertEqual(request.sort_order, "descending")
